<%-- 
    Document   : recep_register_action
    Created on : Jun 16, 2016, 4:27:23 PM
    Author     : IVS PC5
--%>
        ...
        <%   ResultSet rs=null,r=null;
           ...
           int hos_id=Integer.parseInt(session.getAttribute("hos_id").toString());
            ...
            String pass=name+ran+mob.substring(3,7);
             String s="select * from receptionist where mobile='"+mob+"' or email='"+email+"'";
           r=dbc.getData(s);
           if(r.next()){
               JOptionPane.showMessageDialog(null, "Already exists","Error",JOptionPane.INFORMATION_MESSAGE);
           }else{

           String sql="insert into receptionist(name,house,place,post_office,district,mobile,email,hospital_id) values ('"+name+"','"+dep+"','"+qual+"','"+exp+"','"+dist+"','"+mob+"','"+email+"','"+hos_id+"')";
           int i=dbc.putData(sql);
           
           if(i>0){
              String sq1="select id from receptionist where mobile='"+mob+"' and email='"+email+"'";
              rs=dbc.getData(sq1);
              int id=0;
              while(rs.next()){
                  id=rs.getInt("id");
               ...
               String sql1="insert into login(username,password,user_type,status,hospital_id,user_id) values ('"+name+"','"+pass+"','receptionist','0','"+hos_id+"','"+id+"')";
